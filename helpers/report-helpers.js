...
module.exports = {
  getTotalSales: () => {
    return new Promise(async (resolve, reject) => {
      ...
      objR.percDiff = (
        ((objR.todaySale - yesterSale) / yesterSale) *
        100
      ).toFixed(2);
      ...
      objR.percDiffOrder = (
        ((objR.todayOrder - objR.yesterOrder) / objR.yesterOrder) *
        100
      ).toFixed(2);
      ...
      objR.percDiffSearch = (
        ((objR.todaySearch - objR.yesterSearch) / objR.yesterSearch) *
        100
      ).toFixed(2);

      let catSale = await db
        .get()
              ...
              total: {
                $sum: {
                  $multiply: [
                    {
                      $cond: {
                        if: { $isArray: "$products.qty" },
                        then: { $arrayElemAt: ["$products.qty", 0] },
                        else: "$products.qty",
                      },
                    },
                    "$productInfo.price",
    ...
    async function findSaleSum(date) {
      ...
      const sum = await totalArr.reduce((sum, element) => {
    ...
    async function findSearchNum(date) {
      let doc = await db
        .get()
        .collection(collections.SEARCH_COLLECTION)
        .findOne({ date: date });
    ...
    async function findPayNum() {
      ...
      raz = await db
        .get()
        .collection(collections.ORDER_COLLECTION)
        .find({ payMethod: "Razorpay" })
        .toArray();
      pay = await db
        .get()
        .collection(collections.ORDER_COLLECTION)
        .find({ payMethod: "Paypal" })
        .toArray();
  ...
  searches: async () => {
    ...
    if (searchDay) {
      db.get()
        .collection(collections.SEARCH_COLLECTION)
        .updateOne({ date: today }, { $inc: { search: 1 } });
    } else {
  ...
  removePendings: () => {
    db.get()
      .collection(collections.ORDER_COLLECTION)
      .deleteMany({ status: "payment pending" });
  },
};
