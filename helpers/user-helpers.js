...
module.exports = {
  ...
  active: (mob) => {
    return new Promise(async (resolve, reject) => {
      await db
        .get()
        .collection(collections.USER_COLLECTIONS)
        .updateOne({ mobile: mob }, { $set: { active: true } });
      resolve(true);
    });
  },
  inActive: (mob) => {
    return new Promise(async (resolve, reject) => {
      await db
        .get()
        .collection(collections.USER_COLLECTIONS)
        .updateOne({ mobile: mob }, { $set: { active: false } });
      resolve(true);
    });
  },
  blockUser: (mob) => {
    return new Promise(async (resolve, reject) => {
      await db
        .get()
        .collection(collections.USER_COLLECTIONS)
        .updateOne({ mobile: mob }, { $set: { blocked: true } });
      resolve(true);
    });
  },
  unblockUser: (mob) => {
    return new Promise(async (resolve, reject) => {
      await db
        .get()
        .collection(collections.USER_COLLECTIONS)
        .updateOne({ mobile: mob }, { $set: { blocked: false } });
      resolve(true);
    });
  },
  addWishlist: (proId, user) => {
    return new Promise(async (resolve, reject) => {
      let checkWish = await db
        .get()
        .collection(collections.USER_COLLECTIONS)
        .findOne({ mobile: user, wishlist: { $in: [ObjectId(proId)] } });
      if (!checkWish) {
        db.get()
          .collection(collections.USER_COLLECTIONS)
          .updateOne(
  ...
  getAllWishlist: (userMob) => {
    return new Promise(async (resolve, reject) => {
      let wishArr = await db
        .get()
        .collection(collections.USER_COLLECTIONS)
        .aggregate([
          { $match: { mobile: userMob } },
          { $unwind: "$wishlist" },
          {
            $lookup: {
              from: "product",
  ...
  removeWishlist: (proId, user) => {
    return new Promise((resolve, reject) => {
      db.get()
        .collection(collections.USER_COLLECTIONS)
        .updateOne({ mobile: user }, { $pull: { wishlist: ObjectId(proId) } });
      resolve(true);
    });
  },
  addCart: (userMob, proId) => {
    return new Promise(async (resolve, reject) => {
      ...
      let checkCart = await db
        ...
        .findOne({
          mobile: userMob,
          cart: { $elemMatch: { proId: ObjectId(proId) } },
        });
      ...
      if (!checkCart) {
        db.get()
          ...
          .updateOne(
            { mobile: userMob },
            { $push: { cart: { proId: ObjectId(proId), qty: Number(1) } } }
          );
      }
      let wish = await db
        .get()
        .collection(collections.USER_COLLECTIONS)
        .findOne({ mobile: userMob });
      wish = wish?.wishlist;
        ...
        if (!cartArr[i].cartItems[0].deleted) {
          ...
          db.get()
            ...
            .updateOne(
              ...
              {
                $pull: {
                  cart: { proId: ObjectId(cartArr[i].cartItems[0]._id) },
                },
              }
            );
        }
  ...
  removeCart: (proIdn, user) => {
    return new Promise((resolve, reject) => {
      db.get()
        ...
        .updateOne(
          { mobile: user },
          { $pull: { cart: { proId: ObjectId(proIdn) } } }
        );
      resolve(true);
    });
  },
  ...
  updatePass: (body, mob) => {
    return new Promise(async (resolve, reject) => {
      ...
      // console.log(user);
      bcrypt
        .compare(body.currentPassword, user.password)
        .then(async (status) => {
          if (status) {
  ...
  changePayStatus: (id) => {
    return new Promise(async (resolve, reject) => {
      await db
        .get()
        .collection(collections.ORDER_COLLECTION)
        .updateOne({ _id: ObjectId(id) }, { $set: { status: "processing" } });
      let order = await db
        .get()
        .collection(collections.ORDER_COLLECTION)
        .findOne({ _id: ObjectId(id) });
